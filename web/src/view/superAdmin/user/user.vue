<template>
  <div>
    <warning-bar title="Note: You can switch roles by pulling down the avatar in the upper right corner" />
    <div class="gva-table-box">
      <div class="gva-btn-list">
        <el-button size="small" type="primary" icon="plus" @click="addUser">New User</el-button>
      </div>
      <el-table
        :data="tableData"
        row-key="ID"
      >
        ...
        <el-table-column align="left" label="Username" min-width="150" prop="userName" />
        <el-table-column align="left" label="NickName" min-width="150" prop="nickName" />
        ...
        <el-table-column align="left" label="Authorities" min-width="200">
          <template #default="scope">
            <el-cascader
        ...
        <el-table-column align="left" label="Status" min-width="150">
          <template #default="scope">
            <el-switch
        ...
        <el-table-column label="Operations" min-width="250" fixed="right">
          <template #default="scope">
            <el-popover v-model="scope.row.visible" placement="top" width="160">
              <p>Are you sure you want to delete this user</p>
              <div style="text-align: right; margin-top: 8px;">
                <el-button size="small" type="primary" link @click="scope.row.visible = false">Cancel</el-button>
                <el-button type="primary" size="small" @click="deleteUserFunc(scope.row)">Yes</el-button>
              </div>
              <template #reference>
    ...
    >
      <div style="height:60vh;overflow:auto;padding:0 12px;">
        <el-form ref="userForm" :rules="rules" :model="userInfo" label-width="80px">
          <el-form-item v-if="dialogFlag === 'add'" label="Username" prop="userName">
          ...
          <el-form-item label="Authorities" prop="authorityId">
            <el-cascader
          ...
          <el-form-item label="Enable" prop="disabled">
            <el-switch
          ...
          <el-form-item label="Avatar" label-width="80px">
            <div style="display:inline-block" @click="openHeaderChange">
              ...
              <div v-else class="header-img-box">Choose avatar</div>
            </div>
          </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button size="small" type="primary" @click="enterAddUserDialog">Add user</el-button>
        </div>
      </template>
    </el-dialog>
  ...
  userForm.value.validate(async valid => {
    if (valid) {
      ...
      if (dialogFlag.value === 'add') {
        ...
        if (res.code === 0) {
          ElMessage({ type: 'success', message: 'Create successfully' })
          await getTableData()
          closeAddUserDialog()
        }
      }
      if (dialogFlag.value === 'edit') {
        ...
        if (res.code === 0) {
          ElMessage({ type: 'success', message: 'Edit successfully' })
          await getTableData()
          closeAddUserDialog()
        }
      }
...
const changeAuthority = async(row, flag) => {
  ...
  if (res.code === 0) {
    ElMessage({ type: 'success', message: 'Role set successfully' })
  }
}
...
const switchEnable = async(row) => {
  ...
  const res = await setUserInfo(req)
  if (res.code === 0) {
    ElMessage({ type: 'success', message: `${req.enable === 2 ? 'Disable' : 'Enable'} Successfully` })
    await getTableData()
    userInfo.value.headerImg = ''
    userInfo.value.authorityIds = []
  }
