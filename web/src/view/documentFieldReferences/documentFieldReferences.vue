  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="fieldId字段" prop="fieldId" width="120" />
        <el-table-column align="left" label="documentId字段" prop="documentId" width="120" />
        <el-table-column align="left" label="Hành động">
          <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateDocumentFieldReferencesFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteDocumentFieldReferences,
  deleteDocumentFieldReferencesByIds,
  updateDocumentFieldReferences,
  findDocumentFieldReferences,
  getDocumentFieldReferencesList
...
const getTableData = async() => {
  const table = await getDocumentFieldReferencesList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
  ElMessageBox.confirm('Bạn có chắc chắn muốn xoá?', 'Cảnh báo', {
    ...
    deleteDocumentFieldReferencesFunc(row)
  })
}
...
const onDelete = async() => {
  ...
  const res = await deleteDocumentFieldReferencesByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
...
// Update row
const updateDocumentFieldReferencesFunc = async(row) => {
  const res = await findDocumentFieldReferences({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// Delete row
const deleteDocumentFieldReferencesFunc = async(row) => {
  const res = await deleteDocumentFieldReferences({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: 'Xoá thành công'
