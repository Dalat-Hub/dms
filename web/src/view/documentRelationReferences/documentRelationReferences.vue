  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="destId字段" prop="destId" width="120" />
        <el-table-column align="left" label="relationType字段" prop="relationType" width="120" />
        <el-table-column align="left" label="Hành động">
          <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateDocumentRelationReferencesFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteDocumentRelationReferences,
  deleteDocumentRelationReferencesByIds,
  updateDocumentRelationReferences,
  findDocumentRelationReferences,
  getDocumentRelationReferencesList
...
const getTableData = async() => {
  const table = await getDocumentRelationReferencesList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
  ElMessageBox.confirm('Bạn có chắc chắn muốn xoá?', 'Cảnh báo', {
    ...
    deleteDocumentRelationReferencesFunc(row)
  })
}
...
const onDelete = async() => {
  ...
  const res = await deleteDocumentRelationReferencesByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
...
// Update row
const updateDocumentRelationReferencesFunc = async(row) => {
  const res = await findDocumentRelationReferences({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// Delete row
const deleteDocumentRelationReferencesFunc = async(row) => {
  const res = await deleteDocumentRelationReferences({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: 'Xoá thành công'
