      ...
      >
        ...
        <el-table-column align="left" label="Hành động">
          <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateDocumentSignerReferencesFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteDocumentSignerReferences,
  deleteDocumentSignerReferencesByIds,
  updateDocumentSignerReferences,
  findDocumentSignerReferences,
  getDocumentSignerReferencesList
...
const getTableData = async() => {
  const table = await getDocumentSignerReferencesList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
  ElMessageBox.confirm('Bạn có chắc chắn muốn xoá?', 'Cảnh báo', {
    ...
    deleteDocumentSignerReferencesFunc(row)
  })
}
...
const onDelete = async() => {
  ...
  const res = await deleteDocumentSignerReferencesByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
...
// Update row
const updateDocumentSignerReferencesFunc = async(row) => {
  const res = await findDocumentSignerReferences({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// Delete row
const deleteDocumentSignerReferencesFunc = async(row) => {
  const res = await deleteDocumentSignerReferences({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: 'Xoá thành công'
