<template>
  <div>
    ...
    <div class="gva-table-box">
      <div class="gva-btn-list">
        ...
        <el-popover v-model:visible="deleteVisible" placement="top" width="160">
          <p>Xoá những lĩnh vực đã chọn?</p>
      ...
      >
        ...
        <el-table-column align="left" label="Hành động">
          <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateDocumentFieldsFunc(scope.row)">Sửa</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="Hộp thoại lĩnh vực">
...
import {
  ...
  deleteDocumentFieldsByIds,
  updateDocumentFields,
  findDocumentFields,
  getDocumentFieldsList
} from '@/api/documentFields'
...
const getTableData = async() => {
  const table = await getDocumentFieldsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteDocumentFieldsByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
...
// Update row
const updateDocumentFieldsFunc = async(row) => {
  const res = await findDocumentFields({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.redocumentFields
    dialogFormVisible.value = true
  }
}

// Delete row
const deleteDocumentFieldsFunc = async(row) => {
  const res = await deleteDocumentFields({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: 'Xoá thành công'
...
const enterDialog = async() => {
     elFormRef.value?.validate(async(valid) => {
       ...
       switch (type.value) {
         case 'create':
           res = await createDocumentFields(formData.value)
           break
         case 'update':
           res = await updateDocumentFields(formData.value)
           break
         default:
           res = await createDocumentFields(formData.value)
           break
