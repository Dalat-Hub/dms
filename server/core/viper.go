		...
		if config == "" { // 判断命令行参数是否为空
			if configEnv := os.Getenv(internal.ConfigEnv); configEnv == "" { // 判断 internal.ConfigEnv 常量存储的环境变量是否为空
				...
				case gin.DebugMode:
					...
					fmt.Printf("You are using %s environment name in gin mode, the path of config is %s\n", gin.EnvGinMode, internal.ConfigDefaultFile)
			...
			fmt.Printf("You are using the value passed in the -c parameter of the command line, the path to config is %s\n", config)
		}
